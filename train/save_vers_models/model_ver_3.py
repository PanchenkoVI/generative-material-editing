...
class OminiModelFIll(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if ((self.global_step + 1) % self.val_every_n_steps == 0
            ...
            print("\ntrain/loss", self.log_loss, "train/loss_sd", self.res["loss_sd"].detach(), "train/loss_mask",  self.res["loss_mask"].detach(), "train/loss_lpips", self.res["loss_lpips"].detach(), "train/loss_fft", self.res["loss_fft"].detach())
    ...
    def step(self, batch):
        ...
        #     print(
        #         f"[debug] x_t_spatial: {x_t_spatial.shape}, "
        #         f"res_add: {res_add.shape}, "
        #         f"x_t_injected: {x_t_injected.shape}, "
        #         f"masked_latents: {masked_latents.shape}"
        #     )
        ...
        # LPIPS + FFT — один decode на оба loss
        if self.lpips_fn is not None and self.lpips_weight > 0:
            try:
                ...
                # **Вес 0.1** намеренно небольшой — FFT loss работает в пространстве пикселей и его масштаб другой. 
                # Если после 500 шагов `loss_fft` в логах будет близко к нулю — подними до 0.2-0.3. 
                # Если наоборот доминирует — опусти до 0.05.
