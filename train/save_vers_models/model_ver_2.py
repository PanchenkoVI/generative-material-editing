...
class MaterialFusionAdapter(torch.nn.Module): 
    def __init__(self, spatial_dim: int = 128, mat_dim: int = 512, num_heads: int = 4):
        ...
        # ИСПРАВЛЕНИЕ: xavier вместо zeros — адаптер активен с первого шага
        torch.nn.init.xavier_uniform_(self.attn.out_proj.weight, gain=0.5)
        torch.nn.init.zeros_(self.attn.out_proj.bias)

        # Learnable scale: стартует с 0.5 (умеренное влияние)
    ...
    def forward(self, spatial_tokens: torch.Tensor,
        ...
        """
        spatial_tokens : [B, seq, 128]
        global_vec     : [B, feat_dim]
        patch_tokens   : [B, 64, feat_dim]  ← ключевое изменение
        """
        # FiLM: additive bias от глобального цвета
        film_bias = self.film_proj(global_vec).unsqueeze(1)       # [B, 1, 128]
        x = spatial_tokens +  self.film_scale * film_bias

        # Cross-attention: query=spatial, kv=patch токены текстуры
        kv  = self.proj_m(self.norm_m(patch_tokens))              # [B, 64, 128]
        q   = self.norm_s(x)
...
class MaterialCNNEncoder(torch.nn.Module):
    """
    [B, 3, H, W] → (global_vec [B, feat_dim], patch_tokens [B, 64, feat_dim])

    global_vec    — средний цвет/тон (как раньше, для цветового соответствия)
    patch_tokens  — 8×8=64 патча с локальной текстурой (НОВОЕ, для узоров)

    При входе 256×256: 256→128→64→32 → [B, feat_dim, 32, 32]
    AdaptiveAvgPool 8×8 → 64 токена с разной текстурой
    """
    ...
    def forward(self, x: torch.Tensor):
        feat = self.backbone(x)                              # [B, feat_dim, 16, 16]
        global_vec    = self.norm_global(feat.mean(dim=[2, 3]))       # [B, feat_dim]
        
        # patch_spatial = F.adaptive_avg_pool2d(feat, (8, 8))           # [B, feat_dim, 8, 8]
        # patch_tokens  = self.norm_patch(
        #     patch_spatial.flatten(2).permute(0, 2, 1)                 # [B, 64, feat_dim]
        # )
...
class OminiModelFIll(L.LightningModule):
    ...
    def __init__(
        ...
        mat_feat_dim: int = 512,    # размерность MaterialCNNEncoder выхода
    ):
        super().__init__()
    ...
    def configure_optimizers(self):
        ...
        # Обучаем только: LoRA + spatial_cond_encoder + material_cnn_encoder + fusion_adapter
        # cond_proj добавляется лениво через add_param_group
    ...
    @torch.no_grad()
    def clip_score(self, pred_rgb, material_rgb):
        """
        pred_rgb, material_rgb: [B,3,H,W] в диапазоне [-1,1] (как у вас)
        Возвращает средний косинусный сходство между эмбеддингами.
        """
        # Приводим к [0,1] и ресайзим до 224x224
        def prepare(x):
            x = ((x + 1) / 2).clamp(0, 1)
            x = F.interpolate(x, (224, 224), mode="bilinear", align_corners=False)
            return x

        pred_norm = prepare(pred_rgb)
        mat_norm  = prepare(material_rgb)

        # Используем существующий self.ip_image_encoder (CLIPVisionModelWithProjection)
        # Он возвращает image_embeds (проекция CLS токена, размерность 768)
        device = next(self.ip_image_encoder.parameters()).device
        dtype = self.model_dtype
        pred_emb = self.ip_image_encoder(pred_norm.to(device=device, dtype=torch.float32)).image_embeds
        mat_emb  = self.ip_image_encoder(mat_norm.to(device=device, dtype=torch.float32)).image_embeds

        # Нормализуем и считаем косинусное сходство
        pred_emb = F.normalize(pred_emb, dim=-1)
        mat_emb  = F.normalize(mat_emb, dim=-1)
        cos_sim = (pred_emb * mat_emb).sum(dim=-1).mean().item()
        return cos_sim
    ...
    def _build_conditioning_patch(self, materials, edges, mask_proc, B, H, W, device):
        """
        Замена для _build_conditioning в OminiModelFIll.
        Единственное изменение: encoder теперь возвращает (global_vec, patch_tokens)
        и adapter принимает оба.
        """
        ...
        # Ресайзим материал до 256 для CNN
        mat_input = F.interpolate(materials[:, :3], size=(256, 256),
                                mode="bilinear", align_corners=False)
        global_vec, patch_tokens = self.material_cnn_encoder(mat_input)
        # global_vec: [B, 512], patch_tokens: [B, 64, 512]

        # Fusion с patch-токенами
    ...
    def step(self, batch):
        ...
        # 1. Создаём cond_proj, если он ещё не инициализирован
        ...
        # 2. Теперь, когда cond_proj точно есть, загружаем отложенные веса (если они были сохранены)
        ...
        # 3. Применяем cond_proj к объединённому кондишену
        ...
        # LPIPS (только если доступен и нет OOM)
    ...
    def _run_val_visualization(self):
        ...
        if hasattr(self, 'clip_score'):
            try:
                clip_sim = self.clip_score(pred_images, materials[:, :3])
                print(f"  [val {step}] CLIP Score: {clip_sim:.4f}")
                if tb: tb.add_scalar("val/clip_score", clip_sim, step)
        ...
        if tb is not None:
            ...
            grid = torch.cat([mat_disp.cpu(), edge_disp.cpu(),
                              ref_disp.cpu(), pred_disp.cpu(), gt_disp.cpu()], dim=0)
